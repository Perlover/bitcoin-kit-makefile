...
`lnd@.service`'s `ExecStop=` must **wait for lnd to finish shutting down**, not just ask it to. `lncli stop` returns as soon as the shutdown is *initiated*, and systemd SIGTERMs the cgroup the moment `ExecStop=` exits — which used to kill lnd ~1 s into a shutdown that needs ~2-4 s, leaving the log without its final `LTND: Shutdown complete` and the channel databases closed by force. Hence the `kill -0 $MAINPID` poll loop after `lncli stop`. Keep that property in any rewrite; on a Lightning node a truncated shutdown is a channel-state risk, not a cosmetic one. `bitcoind@.service` carries the same loop for the same reason (`bitcoin-cli stop` is equally asynchronous; there `$MAINPID` comes from `PIDFile=`, since bitcoind daemonizes itself). Verifying either fix is a one-liner: a clean lnd stop must end with `LTND: Shutdown complete` in `~/.lnd/logs/bitcoin/<net>/lnd.log`.

`lnd@.service`'s `ExecStart=` redirects lnd's stdout/stderr (`>> ~/.lnd/<net>-lnd-run.log 2>&1` inside `bash -lc`) so the same long-running, append-only log the terminal wrapper has always maintained keeps growing across restarts. The redirection is done by the shell before `exec`, so `$MAINPID` still resolves to lnd. The tradeoff: lnd's stdout/stderr do not reach journald — `journalctl --user -u lnd@<net>` will only show systemd lifecycle events. Use `~/.lnd/<net>-lnd-run.log` (full history) or `~/.lnd/logs/bitcoin/<net>/lnd.log` (lnd's own rotated log) for the daemon's output.
