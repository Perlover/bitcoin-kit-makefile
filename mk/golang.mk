...
# New 1.22.3 requires 4-stage process :(

GOLANG_VER_STAGE_1 := 1.4
GOLANG_VER_STAGE_2 := 1.19.5
GOLANG_VER_STAGE_3 := 1.20.3
GOLANG_VER_STAGE_4 := $(GOLANG_VER)

GOLANG_TARGET_STAGE_2 := $(BASE_INSTALL_DIR)/go$(GOLANG_VER_STAGE_2)/bin/go
GOLANG_TARGET_STAGE_3 := $(BASE_INSTALL_DIR)/go$(GOLANG_VER_STAGE_3)/bin/go
...
$(CURRENT_GOLANG_TARGET): | $(GOLANG_TARGET_STAGE_3)
	...
	cd $(BASE_INSTALL_DIR) && git clone --local $(BASE_INSTALL_DIR)/go$(GOLANG_VER_STAGE_1) go$(GOLANG_VER) && cd go$(GOLANG_VER) && git checkout go$(GOLANG_VER) && cd src && ulimit -u `ulimit -H -u` && GOROOT_BOOTSTRAP=$(BASE_INSTALL_DIR)/go$(GOLANG_VER_STAGE_3) ./make.bash
...
golang_envs-$(GOLANG_VER).sh: golang_envs.sh
	...
	sed -ri -e 's#\$$\$$GOLANG_VER_STAGE_3\$$\$$#$(GOLANG_VER_STAGE_3)#g' $@
